...
# Users can still opt-in to Cloudflare DNS using: arcalos-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-arcalos-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-arcalos-dns.network
  ...
  echo "To use Cloudflare DNS, run: arcalos-setup-dns Cloudflare"
fi