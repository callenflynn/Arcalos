...
# Move user-added backgrounds from arcalos theme folders to user config
ARCALOS_DIR="$HOME/.local/share/arcalos"
USER_BACKGROUNDS_DIR="$HOME/.config/arcalos/backgrounds"

if [[ -d $ARCALOS_DIR/themes ]]; then
  cd "$ARCALOS_DIR"

  # Get list of git-tracked background files (relative to arcalos dir)
...
THEMES_DIR="$HOME/.config/arcalos/themes"
CURRENT_THEME_LINK="$HOME/.config/arcalos/current/theme"
...
elif [[ -f $HOME/.config/arcalos/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/arcalos/current/theme.name")
fi

# Remove all symlinks from ~/.config/arcalos/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  arcalos-theme-set "$CURRENT_THEME_NAME"
else
  # Backup to ensure a theme is set if we can't deduce the name
  arcalos-theme-set "Tokyo Night"
fi
